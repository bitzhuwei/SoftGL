...
namespace SoftGL
{
    ...
    {
        private unsafe List<Fragment> LinearInterpolationTrianglesAdjacency(int count, DrawElementsType type, IntPtr indices, VertexArrayObject vao, ShaderProgram program, GLBuffer indexBuffer, PassBuffer[] passBuffers)
        {
            ...
            count = (count - count % 6);
            for (int indexID = indices.ToInt32() / ByteLength(type), c = 0; c < count - 5 && indexID < indexLength - 5; indexID += 6, c += 6)
            {
