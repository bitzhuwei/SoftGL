...
namespace SoftGL
{
    ...
    {
        private static vec3[] PixelsAtTriangle(vec3 p0, vec3 p1, vec3 p2)
        {
            // use a better(convenient) coordinate.
            p0 += new vec3(0.5f); p1 += new vec3(0.5f); p2 += new vec3(0.5f);
            var x0Integer = (int)Math.Ceiling(p0.x);
            var x1Integer = (int)p1.x;
            var y0Integer = (int)Math.Ceiling(p0.y);
            var y1Integer = (int)p1.y;
            var min = Math.Min(x0Integer, x1Integer);
            var max = Math.Max(x0Integer, x1Integer);
            var xPoints = new vec3[max - min + 1];
            ...
            min = Math.Min(y0Integer, y1Integer);
            max = Math.Max(y0Integer, y1Integer);
            var yPoints = new vec3[max - min + 1];
