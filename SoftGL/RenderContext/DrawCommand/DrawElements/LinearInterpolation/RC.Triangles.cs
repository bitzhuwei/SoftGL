...
namespace SoftGL
{
    ...
    {
        private unsafe List<Fragment> LinearInterpolationTriangles(int count, DrawElementsType type, IntPtr indices, VertexArrayObject vao, ShaderProgram program, GLBuffer indexBuffer, PassBuffer[] passBuffers)
        {
            ...
            count = (count - count % 3);
            for (int indexID = indices.ToInt32() / ByteLength(type), c = 0; c < count - 2 && indexID < indexLength - 2; indexID += 3, c += 3)
            {
